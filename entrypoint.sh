...
if [ -z "${ALERTMANAGER_CONFIG_S3_BUCKET}" -o -z "${ALERTMANAGER_CONFIG_S3_PREFIX}" ]; then
  echo "ERROR: ALERTMANAGER_CONFIG_S3_BUCKET and ALERTMANAGER_CONFIG_S3_PREFIX environment variables must be provided"
  exit 1
fi

S3_URI="s3://${ALERTMANAGER_CONFIG_S3_BUCKET}/${ALERTMANAGER_CONFIG_S3_PREFIX}"
...
echo "INFO: Copying alertmanager configuration file(s) from ${S3_URI} to /etc/alertmanager..."
aws ${PROFILE_OPTION} s3 cp ${S3_URI}/config.yml /etc/alertmanager/config.yml
